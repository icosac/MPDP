...
def main(data_path, model_save_path=MODEL_NAME):
    ...
    if DO_TRAINING:
        ...
        # now = time.time()
        sin, cos, rad, deg = inference_model.predict(np.array([1.7, 1, 0.1, 0.1, np.sin(5.0*np.pi/12.0), np.cos(5.0*np.pi/12.0), np.sin(-4.0*np.pi/12.0), np.cos(-4.0*np.pi/12.0)]))
