...
void test_set(
    ...
    std::vector<std::tuple<
        real_t, real_t, real_t, real_t, real_t, real_t, int, real_t
    >> tests;
    ...
    while (testset >> kmax >> theta_i >> theta_f >> alpha_m >> alpha_f >> th_m >> id_man_comb_f >> len) {
        ...
        tests.push_back(std::make_tuple(kmax, theta_i, theta_f, alpha_m, alpha_f, th_m, id_man_comb, len));
    }

    // Parallel execution of the queries
    #pragma omp parallel for
    for (int i = 0; i < tests.size(); i++) {
        auto [kmax, theta_i, theta_f, alpha_m, alpha_f, th_m, id_man_comb, len] = tests[i];
        std::vector<real_t> query = {kmax, theta_i, theta_f, alpha_m, alpha_f};
        auto start_query_time = std::chrono::high_resolution_clock::now();
        auto predicted_labels = predict_query(annoy_index, query, labels, k, knn_index);
        auto end_query_time = std::chrono::high_resolution_clock::now();
        double query_time = std::chrono::duration_cast<std::chrono::microseconds>(end_query_time - start_query_time).count();
        #pragma omp atomic
...
int main() {
    ...
    switch(RUN_TESTS){
        ...
        case 3:
            ...
            std::cout << "################" << std::endl;
            test_set("/home/enrico/Projects/mpdp/circ_new_test_cc.csv", annoy_index, labels, avg_time, min_time, max_time, 1e0, 1, 1305244);
            avg_time = 0; min_time = 1e9; max_time = 0;  
            std::cout << "################" << std::endl;
            test_set("/home/enrico/Projects/mpdp/circ_new_test_cc.csv", annoy_index, labels, avg_time, min_time, max_time, 1e2, 2, 1305244);
            avg_time = 0; min_time = 1e9; max_time = 0;  
            std::cout << "################" << std::endl;
            test_set("/home/enrico/Projects/mpdp/circ_new_test_cc.csv", annoy_index, labels, avg_time, min_time, max_time, 1e2, 3, 1305244);
            avg_time = 0; min_time = 1e9; max_time = 0;  
            std::cout << "################" << std::endl;
            test_set("/home/enrico/Projects/mpdp/circ_new_test_cc.csv", annoy_index, labels, avg_time, min_time, max_time, 1e3, 4, 1305244);
            avg_time = 0; min_time = 1e9; max_time = 0;  
            std::cout << "################" << std::endl;
            test_set("/home/enrico/Projects/mpdp/circ_new_test_cc.csv", annoy_index, labels, avg_time, min_time, max_time, 1e5, 5, 1305244);
            avg_time = 0; min_time = 1e9; max_time = 0;  
            std::cout << "################" << std::endl;
            // }
        ...
        case 2:
            avg_time = 0; min_time = 1e9; max_time = 0;
            test_set("/home/enrico/Projects/mpdp/examples/3PMD/prediction/datasets/testset2_new.csv", annoy_index, labels, avg_time, min_time, max_time, 1e5, 3, 2104189);
        default:
            break;
    }
