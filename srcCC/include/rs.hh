/*!
 * \file rs.hh
 * \brief Reeds-Shepp curve implementation
 * \author Enrico Saccon <enricosaccon96[at]gmail[dot]com>
 * \copyright Copyright (c) 2023 Enrico Saccon. All rights reserved.
 * \brief This file contains the header for the Reeds-Shepp curve implementation.
 */
#ifndef MPMD_RS_HH
#define MPMD_RS_HH
...
struct RSSegment {
	...
	RSSegment (
			double _x, double _y, double _thi, double _thf, double _l, double _k, int _dir)
	...
	{
		std::stringstream ss;
		ss << "x: " << x << ", y: " << y << ", thi: " << thi << ", thf: " << thf
			 << ", l: " << l << ", k: " << k << ", dir: " << dir;
		return ss.str();
	}

	friend std::ostream&
	operator<< (std::ostream& os, const RSSegment& seg)
	{
		os << seg.toString();
		return os;
	}
...
class RS : public Curve {
 ...
 public:
	RS() : Curve (CURVE_TYPE::RS), _kmax (0) {}

	/*!
	 * @brief Constructor or the Reed-Shepp curve.
	 * @detail The constructor depends on the number of `params`:
	 *  - 0: then `Kmax` is set to `DEFAULT_KMAX` and the number of the manoeuvre to -1 so
	 * that all the possibilities are considered and computed
	 *  - 1: then `Kmax` is set to `params[0]` and the number of the manoeuvre to -1 so that
	 * all the possibilities are considered and computed
	 *  - 2: then `Kmax` is set to `params[0]` and the number of the manoeuvre to
	 * `params[1]` so that only that particular manoeuvre is considered and computed
	 *  - 5: then `Kmax` is set to `params[0]` and the number of the manoeuvre to
	 * `params[1]` so that only that particular manoeuvre is considered, plus `_L1`, `_L2`
	 * and `_L3` are set so that only the `buildRS` function needs to be called.
	...
	/**
	 * @brief Function that computes the best manoeuvre for the given initial and final
	 * configuration.
	 */
	void
	solve()
	...
	{
		std::string str;
		for (auto s : this->getManType()) { str += s; }
		return str;
	}
	[[nodiscard]] LEN_T
	l() const override
	...
	{
		double l = 0.0;
		for (auto tmp : this->L) { l += std::abs (tmp); }
		return l;
	}

	[[nodiscard]] std::vector<RSSegment>
	getSegmentsData();
	...
	getManTypeS() const
	{
		...
		for (int i = 1; i < this->getNseg(); i++) { ret += this->ManType[i]; }
		return ret;
	}
	...
	to_string (int prec = 16) const
	{
		...
		{
			out << i + 1 << " (x,y,th) => " << this->getX()[i] << "  " << this->getY()[i]
					<< "  " << this->getTH()[i] << " to (x,y,th) => " << this->getX()[i + 1] << "  "
					<< this->getY()[i + 1] << "  " << this->getTH()[i + 1]
	...
	friend std::ostream&
	operator<< (std::ostream& out, const RS& curve)
	{
		out << curve.to_string();
		return out;
	}
