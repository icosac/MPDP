...
////  Curve* solveP2PPointer(Configuration2 ci, Configuration2 cf, std::vector<real_type> &params);
...
private:
  ...
  std::vector<std::vector<Cell> >matrix; ///<The matrix used for the dynamic programming algorithm.
  ...
  */
   ...
   */
  void guessInitialAngles(const uint i, std::vector<Cell>& thPrev, std::vector<Cell>& thCur, const std::vector<Configuration2>& points);
  ...
  solveDPInner (std::vector<Configuration2>& points, std::vector<real_type> &params){
    static_assert(std::is_base_of<Curve, CurveT>::value, "CurveT must be a subclass of Curve");
    ...
    for (uint idx=(points.size()-1); idx>0; --idx){ //Cycle between all points starting from the last one
      ...
      for (int i=0; i<(int)(MATRIX[idx-1].size()); ++i){ //Consider the previous angle
        ...
        for (uint j=0; j<MATRIX[idx].size(); ++j){ //Consider the next angle
          //Compute Dubins
          CurveT curve = CurveT(c0->x(), c0->y(), MATRIX[idx-1][i].th(), c1->x(), c1->y(), MATRIX[idx][j].th(), params);
          LEN_T curL = curve.l();
...
public:
  ...
  solveDP(std::vector<Configuration2>& points, const std::vector<bool>& fixedAngles,
              std::vector<real_type> params, int discr, int nRef, bool saveAngles=true){
    static_assert(std::is_base_of<Curve, CurveT>::value, "CurveT must be a subclass of Curve");
    MATRIX.clear();
