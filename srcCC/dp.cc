...
//} //Namespace DP
//
//void DP::setCurveType(CURVE_TYPE curveType) {
//  DP::curveType = curveType;
//}
//
///*!
...
//}
//
//Curve* DP::solveP2PPointer(Configuration2 ci, Configuration2 cf, std::vector<real_type> &params){
//  Curve* curve;
//  switch (DP::curveType){
//    case CURVE_TYPE::RS: {
...
//          curL=dub.l();
//        }
//        else if(DP::curveType == CURVE_TYPE::RS){
//          RS rs=RS(Configuration2(c0->x(), c0->y(), MATRIX[idx-1][i].th()), Configuration2(c1->x(), c1->y(), MATRIX[idx][j].th()), params);
//          curL=rs.l();
//        }
//        else{
//          throw std::runtime_error("The curve type specified is not valid.");
...
void circles(double x1, double y1, double x2, double y2, double r,
                    std::vector<double> & XC, std::vector<double> & YC) {
  double TOL = 1e-8;
...
void DP::guessInitialAngles(const uint i, std::vector<DP::Cell>& thPrev, std::vector<DP::Cell>& thCur, const std::vector<Configuration2>& points){
  thPrev.clear();
  thCur.clear();
...
void DP::setSamplingAngles(int discr, const std::vector<bool>& fixedAngles, const std::vector<Configuration2>& points){
  MATRIX.clear();
  MATRIX.resize(points.size());
...
void DP::setSamplingAngles(const std::vector<Configuration2>& points, const std::vector<bool>& fixedAngles, double hrange, int hn){
  MATRIX.clear();
  MATRIX.resize(points.size());
