...
### Visualizing C++ runs with the Python dashboard

The HTML dashboard that ships with `pympdp` can now be re-used after running the C++
solver.

1. After invoking `DP::solveDP`, call `DP::exportVisualizationData("dp_snapshot.json")` to
   dump the sampled angles, lengths and optimal chain.
2. From the repository root (or any Python environment where `pympdp` is installed) run:
   ```bash
   python3 -m pympdp.dp.visualize_json dp_snapshot.json --hide-optimal-path
   ```
   Use `--no-browser` if you only want the HTML file, or remove
   `--hide-optimal-path` to let Python sample and render the Dubins curve.

The script produces the same interactive visualization as the Python solver, but it reads
all the data from the JSON snapshot generated by the C++ code, so there is no runtime
overhead during the C++ execution.
